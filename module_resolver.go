...
func requireBindingExportType(node moduleGraphNode, summaries map[moduleKey]moduleSummaryArtifact) (TypeSummary, bool) {
	if required, ok := node.RequireBindings[node.ReturnLocal]; ok {
		...
		if node.ReturnField != "" {
			fieldType, ok := tablePropertyType(exportType, node.ReturnField)
			if !ok {
				return TypeSummary{}, false
			}
	...
	if binding, ok := node.RequireFieldBindings[node.ReturnLocal]; ok {
		...
		fieldType, ok := tablePropertyType(exportType, binding.Field)
		if !ok {
			return TypeSummary{}, false
		}
		if node.ReturnField != "" {
			fieldType, ok = tablePropertyType(fieldType, node.ReturnField)
			if !ok {
				return TypeSummary{}, false
			}
		}
...
func tablePropertyType(summary TypeSummary, name string) (TypeSummary, bool) {
