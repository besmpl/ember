...
| cooperative-host-suspension | explicit resumable hooks and captured callbacks with opaque host tokens, repeated waits, multiple pending invocations, error injection, close invalidation, and VM/Machine parity | Embedded | `TestResumableHooksSupportMultiplePendingInvocationsAndRepeatedWaits`, `TestCapturedCallbackCanSuspendAndResumeOnBothEngines`, `TestRuntimeCloseInvalidatesPendingSuspensions`, `TestHearthShapedEmbeddingContract` |
