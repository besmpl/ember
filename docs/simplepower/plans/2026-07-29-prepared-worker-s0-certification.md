...
- Reacquisition found and repaired seven evidence-harness defects: frozen capture B now verifies the hard 5 ms floor instead of demanding capture A's 10 ms calibration observation; a structurally sub-floor complete three-sample B calibration is discarded for at most three bounded reacquisitions before publication; runtime parity gives contaminated or external-engine-timed-out points up to 300 one-second reacquisition attempts; the intrinsically slow full VM all-37 capture gets a 60-minute Go test timeout while dynamic/prepared captures retain 35 minutes; the worker gates buffer complete repeats, discarding a structurally invalid fit for at most three bounded reacquisitions before publication; each all-37 engine/point now retains three order-rotated trials and fits their median; and the comparator preserves the resulting repeat blocks. The sixth repair closes a structural hole exposed remotely: one positive finite timing per point allowed a VM scheduler pause to produce a formally valid but meaningless slope that escaped structural retry and then failed the ratio gate. The seventh removes a different hidden gate: Cartesian recombination divided slopes acquired in different time blocks, so common whole-repeat host drift could fail an otherwise consistent matched comparison. `raw.tsv` retains every trial; the admission script independently reconstructs each median, slope, and matched-repeat ratio. The three matched ratios keep the existing `1.00/1.05` limits, with their worst value as nearest-rank p90. Rejected samples are never emitted, semantic/protocol or measurement errors still fail immediately, and ratio failures under the frozen blocked comparator are never retried. External-CPU-contaminated, interrupted, incomplete, stale-candidate, or reboot-lost attempts are diagnostics only and do not count toward S0.
- Long performance acquisition is no longer permitted on the developer Mac.
  ...
  frame after Wait has completed. The same attempt's Darwin admission exposed
  the cross-block comparator defect after 36 of 37 cases passed: all three
  matched iterative-Fibonacci embedded/Luau repeats were below `1.00`, while
...
3. **Recertify Luau owner-entry AOT.**
   **3A — capture contract:** freeze `guest_batch_v2`, seeds, checksums, allocation fields, toolchain, the four base points `N={50,500,5000,50000}` with a conservatively calibrated per-case power-of-two call scale, and the median/p90 comparator in `scripts/check-runtime-parity`, `scripts/check-prepared-worker-admission`, and the matching admission tests. Capture A selects every scale against a conservative 10 ms target; capture B freezes A's schedule and must clear the hard 5 ms evidence floor rather than reproduce A's exact calibration observation. B buffers a complete three-sample prescribed calibration and may discard a structurally sub-floor set for at most three bounded reacquisitions before publication; rejected calibration samples are never emitted and measurement errors remain terminal. Each worker all-37 engine/point retains three order-rotated timing trials, fits their median, and preserves every trial in `raw.tsv`; the verifier independently reconstructs point medians and slopes and rejects missing trials or a changed median. Each complete repeat is one acquisition block containing every engine at every point. Worker/Luau and embedded/Luau therefore use the three matched-repeat ratios, not cross-block Cartesian combinations; the second ordered ratio is the median and the third is nearest-rank p90. Each raw row records actual scaled N and fitted slopes normalize back to one guest call; this preserves the evidence floor without forcing already-slow cases through the scale required by exceptionally fast prepared cases. Contaminated or external-engine-timed-out points remain rejected and receive only bounded reacquisition attempts; the intrinsically slow full VM all-37 capture gets a 60-minute test timeout while dynamic/prepared captures retain 35 minutes, avoiding Go's brittle 10-minute default without changing any numeric gate. Worker acquisition additionally buffers each complete fitted repeat and retries a structurally invalid fit at most three times before emitting any of its rows; semantic/protocol errors and valid matched-ratio failures remain terminal.
...
7. **Acquire and decide S0.**
   **7A — paired worker admission:** require the GitHub-hosted Darwin ARM64 and Linux x86-64 jobs to retain independent A/B captures; within each capture derive the median and nearest-rank p90 from the three matched repeat-block ratios; require all-37 correctness, worker and embedded versus Luau median `<=1.00` and p90 `<=1.05`, worker/embedded `<=1.50`, and one exchange per timed Apply.
