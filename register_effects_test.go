...
func instructionRegisterLimit(ins instruction) int {
	...
	case opCallMethodOne:
		...
		count, _ := decodeFixedCallCount(ins.d)
		if count > 0 && ins.a+count+2 > limit {
			limit = ins.a + count + 2
		}
	case opFastCall:
		candidate := ins.c
...
func instructionReadsRegister(ins instruction, register int) bool {
	...
	case opCallLocalOne:
		...
		return ins.b == register || register >= start && register < start+count
	case opCallUpvalueOne:
		...
		return register >= start && register < start+count
	case opCallMethodOne:
		count, _ := decodeFixedCallCount(ins.d)
		return ins.b == register || register >= ins.a+2 && register <= ins.a+1+count
