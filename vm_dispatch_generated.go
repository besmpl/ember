...
// dispatch spec sha256: d16869041f3655e3d3f3805cf4fafab1334e49c0ba4dd6721a07b69db38f94f1
...
type directHandlerID uint8

const (
	directHandlerInvalid                    directHandlerID = 0
	directAdaptiveHandlerCap                                = 96
	directAdaptiveHandlerCount                              = 53
	directFusedHandlerCount                                 = 6
	directGeneratedHandlerCount                             = directAdaptiveHandlerCount + directFusedHandlerCount
	directHandlerNumericForTrace            directHandlerID = directHandlerID(opcodeLimit + directAdaptiveHandlerCount + 0)
	directHandlerNumericForTraceOpcode      opcode          = opcode(directHandlerNumericForTrace)
	directNumericForTraceInstructionCap                     = 16
	directHandlerFixedSelfCall              directHandlerID = directHandlerID(opcodeLimit + directAdaptiveHandlerCount + 1)
	directHandlerFixedSelfCallOpcode        opcode          = opcode(directHandlerFixedSelfCall)
	directFixedSelfCallInstructionCap                       = 1
	directHandlerFixedSelfCallTrace         directHandlerID = directHandlerID(opcodeLimit + directAdaptiveHandlerCount + 2)
	directHandlerFixedSelfCallTraceOpcode   opcode          = opcode(directHandlerFixedSelfCallTrace)
	directFixedSelfCallTraceInstructionCap                  = 3
	directHandlerCompactSelfFunction        directHandlerID = directHandlerID(opcodeLimit + directAdaptiveHandlerCount + 3)
	directHandlerCompactSelfFunctionOpcode  opcode          = opcode(directHandlerCompactSelfFunction)
	directCompactSelfFunctionInstructionCap                 = 16
	directHandlerCompactLeafCall            directHandlerID = directHandlerID(opcodeLimit + directAdaptiveHandlerCount + 4)
	directHandlerCompactLeafCallOpcode      opcode          = opcode(directHandlerCompactLeafCall)
	directCompactLeafCallInstructionCap                     = 24
	directHandlerCompactLoop                directHandlerID = directHandlerID(opcodeLimit + directAdaptiveHandlerCount + 5)
	directHandlerCompactLoopOpcode          opcode          = opcode(directHandlerCompactLoop)
	directCompactLoopInstructionCap                         = 16
)

type directSpecializationFamily uint8

const (
	directSpecializationNone directSpecializationFamily = iota
	directSpecializationGlobalRead
	directSpecializationFieldRead
	directSpecializationFieldWrite
	directSpecializationIndexRead
	directSpecializationIndexWrite
	directSpecializationIteration
	directSpecializationNumberBinary
	directSpecializationNumberConstant
	directSpecializationNumberUnary
	directSpecializationLength
	directSpecializationConcat
	directSpecializationCompare
	directSpecializationNumericLoop
	directSpecializationTableShape
	directSpecializationBuiltinCall
	directSpecializationTruthBranch
	directSpecializationDirectCall
)

type directCacheLayout uint8

const (
	directCacheNone directCacheLayout = iota
	directCacheType
	directCacheGlobal
	directCacheField
	directCacheIndex
	directCacheIteration
	directCacheShape
	directCacheCall
)
...
const (
	directTilingPure directTilingPolicy = iota + 1
	directTilingGuarded
	directTilingTerminal
	directTilingBarrier
)
...
var generatedDirectSemanticMetadata = func() [opcodeLimit]directSemanticMetadata {
	...
	table[opLoadConst] = newDirectSemanticMetadata(opLoadConst, directHandlerInvalid, directSpecializationNone, directTilingPure, directCacheNone)
	table[opLoadGlobal] = newDirectSemanticMetadata(opLoadGlobal, directHandlerID(opcodeLimit+0), directSpecializationGlobalRead, directTilingGuarded, directCacheGlobal)
	table[opSetGlobal] = newDirectSemanticMetadata(opSetGlobal, directHandlerInvalid, directSpecializationNone, directTilingBarrier, directCacheNone)
	table[opNewTable] = newDirectSemanticMetadata(opNewTable, directHandlerInvalid, directSpecializationNone, directTilingBarrier, directCacheNone)
	table[opSetField] = newDirectSemanticMetadata(opSetField, directHandlerID(opcodeLimit+1), directSpecializationFieldWrite, directTilingBarrier, directCacheField)
	table[opSetStringField] = newDirectSemanticMetadata(opSetStringField, directHandlerID(opcodeLimit+2), directSpecializationFieldWrite, directTilingBarrier, directCacheField)
	table[opSetStringFieldIndex] = newDirectSemanticMetadata(opSetStringFieldIndex, directHandlerID(opcodeLimit+3), directSpecializationFieldWrite, directTilingBarrier, directCacheField)
	table[opGetStringField] = newDirectSemanticMetadata(opGetStringField, directHandlerID(opcodeLimit+4), directSpecializationFieldRead, directTilingGuarded, directCacheField)
	table[opGetStringFieldIndex] = newDirectSemanticMetadata(opGetStringFieldIndex, directHandlerID(opcodeLimit+5), directSpecializationFieldRead, directTilingGuarded, directCacheField)
	table[opSetIndex] = newDirectSemanticMetadata(opSetIndex, directHandlerID(opcodeLimit+6), directSpecializationIndexWrite, directTilingBarrier, directCacheIndex)
	table[opGetIndex] = newDirectSemanticMetadata(opGetIndex, directHandlerID(opcodeLimit+7), directSpecializationIndexRead, directTilingGuarded, directCacheIndex)
	table[opClosure] = newDirectSemanticMetadata(opClosure, directHandlerInvalid, directSpecializationNone, directTilingBarrier, directCacheNone)
	table[opGetUpvalue] = newDirectSemanticMetadata(opGetUpvalue, directHandlerInvalid, directSpecializationNone, directTilingPure, directCacheNone)
	table[opSetUpvalue] = newDirectSemanticMetadata(opSetUpvalue, directHandlerInvalid, directSpecializationNone, directTilingBarrier, directCacheNone)
	table[opVararg] = newDirectSemanticMetadata(opVararg, directHandlerInvalid, directSpecializationNone, directTilingBarrier, directCacheNone)
	table[opPrepareIter] = newDirectSemanticMetadata(opPrepareIter, directHandlerID(opcodeLimit+8), directSpecializationIteration, directTilingBarrier, directCacheIteration)
	table[opArrayNext] = newDirectSemanticMetadata(opArrayNext, directHandlerID(opcodeLimit+9), directSpecializationIteration, directTilingGuarded, directCacheIteration)
	table[opArrayNextJump2] = newDirectSemanticMetadata(opArrayNextJump2, directHandlerID(opcodeLimit+10), directSpecializationIteration, directTilingTerminal, directCacheIteration)
	table[opMove] = newDirectSemanticMetadata(opMove, directHandlerInvalid, directSpecializationNone, directTilingPure, directCacheNone)
	table[opAdd] = newDirectSemanticMetadata(opAdd, directHandlerID(opcodeLimit+11), directSpecializationNumberBinary, directTilingGuarded, directCacheType)
	table[opSub] = newDirectSemanticMetadata(opSub, directHandlerID(opcodeLimit+12), directSpecializationNumberBinary, directTilingGuarded, directCacheType)
	table[opMul] = newDirectSemanticMetadata(opMul, directHandlerID(opcodeLimit+13), directSpecializationNumberBinary, directTilingGuarded, directCacheType)
	table[opDiv] = newDirectSemanticMetadata(opDiv, directHandlerID(opcodeLimit+14), directSpecializationNumberBinary, directTilingGuarded, directCacheType)
	table[opMod] = newDirectSemanticMetadata(opMod, directHandlerID(opcodeLimit+15), directSpecializationNumberBinary, directTilingGuarded, directCacheType)
	table[opIDiv] = newDirectSemanticMetadata(opIDiv, directHandlerID(opcodeLimit+16), directSpecializationNumberBinary, directTilingGuarded, directCacheType)
	table[opAddK] = newDirectSemanticMetadata(opAddK, directHandlerID(opcodeLimit+17), directSpecializationNumberConstant, directTilingGuarded, directCacheType)
	table[opSubK] = newDirectSemanticMetadata(opSubK, directHandlerID(opcodeLimit+18), directSpecializationNumberConstant, directTilingGuarded, directCacheType)
	table[opMulK] = newDirectSemanticMetadata(opMulK, directHandlerID(opcodeLimit+19), directSpecializationNumberConstant, directTilingGuarded, directCacheType)
	table[opDivK] = newDirectSemanticMetadata(opDivK, directHandlerID(opcodeLimit+20), directSpecializationNumberConstant, directTilingGuarded, directCacheType)
	table[opModK] = newDirectSemanticMetadata(opModK, directHandlerID(opcodeLimit+21), directSpecializationNumberConstant, directTilingGuarded, directCacheType)
	table[opIDivK] = newDirectSemanticMetadata(opIDivK, directHandlerID(opcodeLimit+22), directSpecializationNumberConstant, directTilingGuarded, directCacheType)
	table[opPow] = newDirectSemanticMetadata(opPow, directHandlerID(opcodeLimit+23), directSpecializationNumberBinary, directTilingGuarded, directCacheType)
	table[opNeg] = newDirectSemanticMetadata(opNeg, directHandlerID(opcodeLimit+24), directSpecializationNumberUnary, directTilingGuarded, directCacheType)
	table[opLen] = newDirectSemanticMetadata(opLen, directHandlerID(opcodeLimit+25), directSpecializationLength, directTilingGuarded, directCacheType)
	table[opConcat] = newDirectSemanticMetadata(opConcat, directHandlerID(opcodeLimit+26), directSpecializationConcat, directTilingGuarded, directCacheType)
	table[opConcatChain] = newDirectSemanticMetadata(opConcatChain, directHandlerID(opcodeLimit+27), directSpecializationConcat, directTilingGuarded, directCacheType)
	table[opEqual] = newDirectSemanticMetadata(opEqual, directHandlerID(opcodeLimit+28), directSpecializationCompare, directTilingGuarded, directCacheType)
	table[opNotEqual] = newDirectSemanticMetadata(opNotEqual, directHandlerID(opcodeLimit+29), directSpecializationCompare, directTilingGuarded, directCacheType)
	table[opLess] = newDirectSemanticMetadata(opLess, directHandlerID(opcodeLimit+30), directSpecializationCompare, directTilingGuarded, directCacheType)
	table[opLessEqual] = newDirectSemanticMetadata(opLessEqual, directHandlerID(opcodeLimit+31), directSpecializationCompare, directTilingGuarded, directCacheType)
	table[opGreater] = newDirectSemanticMetadata(opGreater, directHandlerID(opcodeLimit+32), directSpecializationCompare, directTilingGuarded, directCacheType)
	table[opGreaterEqual] = newDirectSemanticMetadata(opGreaterEqual, directHandlerID(opcodeLimit+33), directSpecializationCompare, directTilingGuarded, directCacheType)
	table[opNumericForCheck] = newDirectSemanticMetadata(opNumericForCheck, directHandlerID(opcodeLimit+34), directSpecializationNumericLoop, directTilingTerminal, directCacheType)
	table[opNumericForLoop] = newDirectSemanticMetadata(opNumericForLoop, directHandlerID(opcodeLimit+35), directSpecializationNumericLoop, directTilingTerminal, directCacheType)
	table[opJumpIfNotEqualK] = newDirectSemanticMetadata(opJumpIfNotEqualK, directHandlerID(opcodeLimit+36), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfNotLessK] = newDirectSemanticMetadata(opJumpIfNotLessK, directHandlerID(opcodeLimit+37), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfNotGreaterK] = newDirectSemanticMetadata(opJumpIfNotGreaterK, directHandlerID(opcodeLimit+38), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfLessK] = newDirectSemanticMetadata(opJumpIfLessK, directHandlerID(opcodeLimit+39), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfGreaterK] = newDirectSemanticMetadata(opJumpIfGreaterK, directHandlerID(opcodeLimit+40), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfNotLess] = newDirectSemanticMetadata(opJumpIfNotLess, directHandlerID(opcodeLimit+41), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfNotGreater] = newDirectSemanticMetadata(opJumpIfNotGreater, directHandlerID(opcodeLimit+42), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfLess] = newDirectSemanticMetadata(opJumpIfLess, directHandlerID(opcodeLimit+43), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfGreater] = newDirectSemanticMetadata(opJumpIfGreater, directHandlerID(opcodeLimit+44), directSpecializationCompare, directTilingTerminal, directCacheType)
	table[opJumpIfTableHasMetatable] = newDirectSemanticMetadata(opJumpIfTableHasMetatable, directHandlerID(opcodeLimit+45), directSpecializationTableShape, directTilingTerminal, directCacheShape)
	table[opFastCall] = newDirectSemanticMetadata(opFastCall, directHandlerID(opcodeLimit+46), directSpecializationBuiltinCall, directTilingBarrier, directCacheCall)
	table[opJumpIfFalse] = newDirectSemanticMetadata(opJumpIfFalse, directHandlerID(opcodeLimit+47), directSpecializationTruthBranch, directTilingTerminal, directCacheType)
	table[opCall] = newDirectSemanticMetadata(opCall, directHandlerID(opcodeLimit+48), directSpecializationDirectCall, directTilingBarrier, directCacheCall)
	table[opCallOne] = newDirectSemanticMetadata(opCallOne, directHandlerID(opcodeLimit+49), directSpecializationDirectCall, directTilingBarrier, directCacheCall)
	table[opCallLocalOne] = newDirectSemanticMetadata(opCallLocalOne, directHandlerID(opcodeLimit+50), directSpecializationDirectCall, directTilingBarrier, directCacheCall)
	table[opCallUpvalueOne] = newDirectSemanticMetadata(opCallUpvalueOne, directHandlerID(opcodeLimit+51), directSpecializationDirectCall, directTilingBarrier, directCacheCall)
	table[opCallMethodOne] = newDirectSemanticMetadata(opCallMethodOne, directHandlerID(opcodeLimit+52), directSpecializationDirectCall, directTilingBarrier, directCacheCall)
	table[opJump] = newDirectSemanticMetadata(opJump, directHandlerInvalid, directSpecializationNone, directTilingTerminal, directCacheNone)
	table[opReturnOne] = newDirectSemanticMetadata(opReturnOne, directHandlerInvalid, directSpecializationNone, directTilingBarrier, directCacheNone)
	table[opReturn] = newDirectSemanticMetadata(opReturn, directHandlerInvalid, directSpecializationNone, directTilingBarrier, directCacheNone)
	return table
}()
...
func runGeneratedDirectFrameInstrumentedLoop(thread *vmThread, frameRef **vmFrame) directFrameSideExit {
...
reload:
	*frameRef = frame
	...
	runLineHook = true && thread.debugHook != nil && thread.debugLineHook
	runCountHook = true && thread.debugCountInterval > 0 && thread.debugHook != nil
	...
	adaptiveDispatch:
		...
		case opSetStringField:
			...
			if valueKind(keyValue) == StringKind && !value.IsNil() {
				...
				if fieldCache.writeCounted(table, value, picCounts) || fieldCache.resolveWriteCounted(table, key, keyValue.stringBox(), value, picCounts, thread.controller) {
					break
				}
		...
		case opArrayNext:
			...
			if true && valueNativeID(callee) == nativeFuncArrayNext {
		...
		case directHandlerCompactLeafCallOpcode:
			adaptiveWord := functionInstance.shadow.words[pc]
			aux := words[nextWord]
			...
			if borrowHint {
				callee := registers[b]
				if value, ok := thread.runDirectCompactLeafCall(frame, callee, registers[b+1:b+1+argCount]); ok {
					directFrameApplySingleCallIslandResult(frame, registers, a, 1, value)
					functionInstance.shadow.words[pc] = adaptiveWord.incrementCounter()
					break
				}
			}
			functionInstance.shadow.words[pc] = adaptiveWord.withHandler(directHandlerID(opCallOne))
			op = opCallOne
			goto adaptiveDispatch

		case directHandlerCompactLoopOpcode:
			compactPC := pc
			adaptiveWord := functionInstance.shadow.words[compactPC]
			plan, planOK := functionInstance.shadow.compactLoopAt(compactPC)
			if !planOK {
				return directFrameExitAt(frame, compactPC, directFrameFail(fmt.Errorf("compact loop at word %d has no execution plan", compactPC)))
			}
			nextPC, errorPC, loopErr, guardOK := thread.runDirectCompactLoop(frame, functionInstance, plan)
			if !guardOK {
				generic := opcode(uint8(adaptiveWord.raw()) & uint8(wordcodeOpcodeMask))
				functionInstance.shadow.words[compactPC] = adaptiveWord.withHandler(directHandlerID(generic))
				pc = nextPC
				continue
			}
			functionInstance.shadow.words[compactPC] = adaptiveWord.incrementCounter()
			if loopErr != nil {
				return directFrameExitAt(frame, errorPC, directFrameFail(loopErr))
			}
			pc = nextPC
			continue
...
func runGeneratedDirectFrameProductionLoop(thread *vmThread, frameRef **vmFrame) directFrameSideExit {
	previousWindow := thread.executionWindow
	thread.executionWindow = newExecutionWindow(thread.controller)
	window := &thread.executionWindow
	defer func() {
	...
	adaptiveDispatch:
		...
		case opCallOne:
			...
			if borrowHint {
				if value, ok := thread.runDirectCompactLeafCall(frame, callee, registers[b+1:b+1+argCount]); ok {
					directFrameApplySingleCallIslandResult(frame, registers, a, 1, value)
					observeDirectCompactLeafCall(functionInstance, pc)
					break
				}
			}
