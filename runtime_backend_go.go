...
func emitBackendGoNumericProof(ir *backendProtoIR, options backendGoNumericOptions) ([]byte, error) {
	...
	if (options.preparedImportPath == "") != (options.preparedQualifier == "") {
	...
	if options.preparedQualifier != "" {
		...
		if !token.IsIdentifier(options.preparedQualifier) ||
			token.Lookup(options.preparedQualifier).IsKeyword() ||
			options.preparedQualifier == "math" {
			return nil, fmt.Errorf("emit backend Go numeric proof: invalid prepared import qualifier %q", options.preparedQualifier)
		}
	}
	...
	if options.preparedQualifier != "" {
		source.WriteString("// Code generated by Ember's prepared compiler; DO NOT EDIT.\n\n")
	} else {
		source.WriteString("// Code generated by Ember's private prepared proof compiler; DO NOT EDIT.\n\n")
	}
	fmt.Fprintf(&source, "package %s\n\n", options.packageName)
	needsMath := directEmitter.needsMath || preparedEmitter.needsMath || coroutineNeedsMath
	if needsMath && options.preparedQualifier != "" {
		fmt.Fprintf(&source, "import (\n\t\"math\"\n\t%s %q\n)\n\n", options.preparedQualifier, options.preparedImportPath)
	} else if needsMath {
		source.WriteString("import \"math\"\n\n")
	} else if options.preparedQualifier != "" {
	...
	if options.preparedFunctionName != "" {
		...
		fmt.Fprintf(&source, "\nfunc %s(context %s", bodyName, options.preparedContextType())
		for parameter := 0; parameter < ir.params; parameter++ {
			fmt.Fprintf(&source, ", p%d float64", parameter)
		}
		fmt.Fprintf(&source, ") %s {\n", options.preparedExitType())
		for valueIndex := range ir.values {
		...
		fmt.Fprintf(&source, "\nfunc %s(context %s) %s {\n", options.preparedFunctionName, options.preparedContextType(), options.preparedExitType())
		for parameter := 0; parameter < ir.params; parameter++ {
			fmt.Fprintf(&source, "\tp%d, ok := context.%s(%d)\n", parameter, options.preparedContextMethod("numberParameter"), parameter)
			fmt.Fprintf(&source, "\tif !ok {\n\t\treturn %s\n\t}\n", options.preparedReplayEntry())
		}
		for pc := range ir.ops {
			operation := &ir.ops[pc]
			...
			fmt.Fprintf(&source, "\tif !context.%s(%d) {\n", options.preparedContextMethod("intrinsicUnchanged"), operation.pc)
		...
		if plan.coroutines.enabled {
			for pc := range plan.coroutines.target.ir.ops {
				...
				fmt.Fprintf(
					&source,
					"\tif !context.%s(%d, %d) {\n",
					options.preparedContextMethod("intrinsicUnchangedAt"),
...
func (options backendGoNumericOptions) preparedContextType() string {
	if options.preparedQualifier != "" {
		return options.preparedQualifier + ".PreparedContext"
	}
	return "machinePreparedContext"
}

func (options backendGoNumericOptions) preparedExitType() string {
	if options.preparedQualifier != "" {
		return options.preparedQualifier + ".PreparedExit"
	}
	return "machinePreparedExit"
}

func (options backendGoNumericOptions) preparedReplayEntry() string {
	if options.preparedQualifier != "" {
		return options.preparedQualifier + ".PreparedReplayEntry()"
	}
	return "machinePreparedReplayEntry()"
}

func (options backendGoNumericOptions) preparedReturnOneNumber() string {
	if options.preparedQualifier != "" {
		return options.preparedQualifier + ".PreparedReturnOneNumber"
	}
	return "machinePreparedReturnOneNumber"
}

func (options backendGoNumericOptions) preparedExitLiteral() string {
	return options.preparedExitType() + "{}"
}

func (options backendGoNumericOptions) preparedContextMethod(name string) string {
	if options.preparedQualifier == "" {
		return name
	}
	switch name {
...
func buildBackendGoNumericPlan(ir *backendProtoIR, options backendGoNumericOptions) (backendGoNumericPlan, error) {
	...
	indexFunctions := analyzeBackendGoRecordIndexFunctions(ir, records, options)
	plan := backendGoNumericPlan{
		...
		for valueIndex := range ir.values {
			...
			case backendValuePhi:
				...
				for inputIndex, input := range phi.inputs {
					...
					inputTags := plan.tags[input-1]
					if backendGoPhiInputComesFromTruthyEdge(ir, value.block, inputIndex, input) {
						inputTags &^= backendTagNil
					}
					tags |= inputTags
				}
			case backendValueOperation:
				...
				case opGetIndex:
					if _, ok := plan.mutationMetatable.get(operation); ok {
						...
						tags = backendTagNumber
					} else if dynamic, ok := plan.tables.dynamicExternalGetByPC[operation.pc]; ok {
						tags = dynamic.tags | backendTagNil
				...
				case opFastCall:
					if plan.mutationMetatable.metatable(operation) {
						...
						tags = backendTagNumber
					} else if metatable, ok := plan.records.metatableOperation(operation); ok {
						if plan.records.root(value.id) == metatable.table {
							tags = backendTagTable
						}
	...
	for valueIndex, function := range plan.records.functionValues {
		if function {
			markScalarReplaced(backendValueID(valueIndex + 1))
		}
	}
...
func backendGoPhiInputComesFromTruthyEdge(
	ir *backendProtoIR,
	phiBlock int32,
	inputIndex int,
	input backendValueID,
) bool {
	if ir == nil || phiBlock < 0 || int(phiBlock) >= len(ir.blocks) ||
		inputIndex < 0 || inputIndex >= len(ir.blocks[phiBlock].predecessors) {
		return false
	}
	pathBlock := ir.blocks[phiBlock].predecessors[inputIndex]
	if pathBlock < 0 || int(pathBlock) >= len(ir.blocks) {
		return false
	}
	path := &ir.blocks[pathBlock]
	if len(path.predecessors) != 1 || path.first+1 != path.last ||
		path.first < 0 || int(path.first) >= len(ir.ops) || ir.ops[path.first].op != opJump {
		return false
	}
	guardBlock := path.predecessors[0]
	if guardBlock < 0 || int(guardBlock) >= len(ir.blocks) {
		return false
	}
	guard := &ir.blocks[guardBlock]
	if guard.last <= guard.first || int(guard.last) > len(ir.ops) {
		return false
	}
	branch := &ir.ops[guard.last-1]
	if branch.op != opJumpIfFalse || backendOperationUse(branch, branch.a) != input {
		return false
	}
	falseBlock := int32(-1)
	if branch.targetPC >= 0 && int(branch.targetPC) < len(ir.pcToBlock) {
		falseBlock = ir.pcToBlock[branch.targetPC]
	}
...
func verifyBackendGoNumericOperation(
	...
	case opSetStringField:
		...
		if plan.records.functionSetter(operation) {
			return nil
		}
	...
	case opGetIndex:
		...
		if dynamic, ok := plan.tables.dynamicExternalGetByPC[operation.pc]; ok {
			if err := requireOptional(operation.c, backendTagString); err != nil {
				return err
			}
			for _, definition := range operation.defs {
				if plan.tags[definition.value-1] != dynamic.tags|backendTagNil {
					return fmt.Errorf("emit backend Go numeric proof: PC %d dynamic captured-table lookup changes scalar tags", operation.pc)
				}
			}
			return nil
		}
	...
	case opGetStringField:
		...
		if _, ok := plan.indexFunctions.call(operation); ok {
			for _, definition := range operation.defs {
				if plan.tags[definition.value-1] != backendTagNumber {
					return fmt.Errorf("emit backend Go numeric proof: PC %d static __index result is not numeric", operation.pc)
				}
			}
			return nil
		}
	...
	case opFastCall:
		...
		if metatable, ok := plan.records.metatableOperation(operation); ok {
			...
			for _, definition := range operation.defs {
				if plan.records.root(definition.value) != metatable.table {
					return fmt.Errorf("emit backend Go numeric proof: PC %d changes record setmetatable identity", operation.pc)
				}
			}
			return nil
		}
...
func (emitter *backendGoNumericEmitter) emitOperation(operation *backendOperationIR, block *backendBlockIR) (bool, error) {
	...
	case opSetStringField:
		...
		if emitter.plan.records.functionSetter(operation) {
			return false, nil
		}
	...
	case opGetIndex:
		...
		if dynamic, ok := emitter.plan.tables.dynamicExternalGetByPC[operation.pc]; ok {
			destination, err := definition(operation.a)
			if err != nil {
				return false, err
			}
...
func (emitter *backendGoNumericEmitter) emitReplayBeforeOperation(operation *backendOperationIR, indent int) {
	...
	if emitter.plan.replayEntry[operation.pc] {
		fmt.Fprintf(&emitter.body, "%sreturn %s\n", prefix, emitter.options.preparedReplayEntry())
		return
	}
	fmt.Fprintf(&emitter.body, "%sexit := context.%s(%d, %d)\n", prefix, emitter.options.preparedContextMethod("replayBeforeOperation"), operation.pc, len(operation.spillValues))
	for spillIndex, spill := range operation.spillValues {
		...
		if payload, optional := backendGoOptionalScalarTags(tags); optional {
			...
			switch payload {
			case backendTagBool:
				fmt.Fprintf(&emitter.body, "%s\tcontext.%s(%d, %d, v%d)\n", prefix, emitter.options.preparedContextMethod("spillBool"), spillIndex, spill.register, spill.value)
			case backendTagNumber:
				fmt.Fprintf(&emitter.body, "%s\tcontext.%s(%d, %d, v%d)\n", prefix, emitter.options.preparedContextMethod("spillNumber"), spillIndex, spill.register, spill.value)
			default:
				fmt.Fprintf(&emitter.body, "%s\treturn %s\n", prefix, emitter.options.preparedExitLiteral())
			}
			fmt.Fprintf(&emitter.body, "%s} else {\n", prefix)
			fmt.Fprintf(&emitter.body, "%s\tcontext.%s(%d, %d)\n", prefix, emitter.options.preparedContextMethod("spillNil"), spillIndex, spill.register)
			fmt.Fprintf(&emitter.body, "%s}\n", prefix)
			continue
		}
		switch tags {
		case backendTagNil:
			fmt.Fprintf(&emitter.body, "%scontext.%s(%d, %d)\n", prefix, emitter.options.preparedContextMethod("spillNil"), spillIndex, spill.register)
		case backendTagBool:
			fmt.Fprintf(&emitter.body, "%scontext.%s(%d, %d, v%d)\n", prefix, emitter.options.preparedContextMethod("spillBool"), spillIndex, spill.register, spill.value)
		case backendTagNumber:
			fmt.Fprintf(&emitter.body, "%scontext.%s(%d, %d, v%d)\n", prefix, emitter.options.preparedContextMethod("spillNumber"), spillIndex, spill.register, spill.value)
		default:
			fmt.Fprintf(&emitter.body, "%sreturn %s\n", prefix, emitter.options.preparedExitLiteral())
...
func (emitter *backendGoNumericEmitter) failureReturn() string {
	if emitter.prepared {
		return "return " + emitter.options.preparedReplayEntry()
	}
