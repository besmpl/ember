...
- Reacquisition found and repaired six evidence-harness defects: frozen capture B now verifies the hard 5 ms floor instead of demanding capture A's 10 ms calibration observation; a structurally sub-floor complete three-sample B calibration is discarded for at most three bounded reacquisitions before publication; runtime parity gives contaminated or external-engine-timed-out points up to 300 one-second reacquisition attempts; the intrinsically slow full VM all-37 capture gets a 60-minute Go test timeout while dynamic/prepared captures retain 35 minutes; the worker gates buffer complete repeats, discarding a structurally invalid fit for at most three bounded reacquisitions before publication; and each all-37 engine/point now retains three order-rotated trials and fits their median. The sixth repair closes a structural hole exposed remotely: one positive finite timing per point allowed a VM scheduler pause to produce a formally valid but meaningless slope that escaped structural retry and then failed the ratio gate. `raw.tsv` retains every trial, and the admission script independently reconstructs each median and slope. Rejected samples are never emitted, semantic/protocol or measurement errors still fail immediately, thresholds are unchanged, and ratio failures are never retried. External-CPU-contaminated, interrupted, incomplete, stale-candidate, or reboot-lost attempts are diagnostics only and do not count toward S0.
